    ...
    <>
      <div id="part-5" className="p-5"  >
          ...
          <div id="slider_count">Salary ={tempSal.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}</div>
            <br/>
          <input onChange={(e)=>{dispatch({type:'SALARYSTATE',payload:e.target.value});setTempSal(e.target.value);}} value={tempSal} type="range" class="form-range" min="10000" max="100000" step="10000" id="customRange1"/>
